...
export function useDrrAll() {
  ...
  useEffect(() => {
    ...
    // Refresh data whenever the user returns focus to the tab. Common
    // scenario: user uploads new DRR on /ingestion route, switches back
    // to / (or to any module view) — without this, the stale module-level
    // cache would still be served. The visibility refresh is cheap (single
    // Supabase fetch) and keeps the UI honest.
    const onVisibility = () => {
      if (document.visibilityState === "visible") {
        refreshDrr().catch(() => { /* noop — keep stale cache on error */ });
      }
    };
  ...
  return {
    rows: cache ?? [],
    loading,
    /** Manually force a re-fetch (e.g. after an in-page upload). */
    refresh: refreshDrr,
  };
}
...
export function useDrrFiltered(selected: string) {
  const { rows, loading, refresh } = useDrrAll();
  ...
  return { rows: filtered, latest, latestPerProperty, loading, allRows: rows, refresh };
}
