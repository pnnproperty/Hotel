...
function VoiceOrbInner({ activeModule }: { activeModule: string }) {
  ...
  const conversation = useConversation({
    clientTools: {
      ...
      //
      // By DEFAULT we open WITHOUT noopener so the tab is tracked and can
      // be closed later by `closeAll`. This is the right trade-off for an
      // agent context where the agent controls the URL.
      //
      // If the agent passes `secure: true`, we add noopener — the tab gets
      // stronger isolation but becomes UNCLOSABLE by us afterwards (browser
      // gives us back null instead of a window reference).
      openExternalLink: (params: { url: string; reason?: string; secure?: boolean }) => {
        try {
          const url = new URL(params.url);
          if (!["http:", "https:"].includes(url.protocol)) {
            return "Only http/https URLs are allowed.";
          }
          // Default: trackable (closable). Opt-in: secure (uncloseable but isolated)
          const features = params.secure ? "noopener,noreferrer" : "";
          const win = window.open(url.toString(), "_blank", features);
          if (win) openedTabsRef.current.push(win);
          return `Opened ${url.hostname} in a new tab${params.secure ? " (secure mode, not closable)" : ""}.`;
        } catch {
      ...
      // Closes modal overlays (video, email, search results, etc) AND any
      // tabs the agent previously opened via openExternalLink (provided
      // they weren't opened in secure mode).
      //
      // Important: Browser security limits what we can close:
      //   - Modal overlays — always closable
      //   - Tabs opened by agent (default mode) — closable
      //   - Tabs opened by agent in secure mode — NOT closable
      //   - User's other tabs (opened manually) — NOT closable
      //   - The main dashboard tab itself — NOT closable
      closeAll: (params: { what?: "modals" | "tabs" | "everything" } = {}) => {
        ...
        // First, exit fullscreen if active — otherwise YouTube fullscreen
        // would persist and hide the cleared modal area visually.
        ...
        const parts: string[] = [];
        if (closedModal) parts.push("closed the overlay");
        ...
        if (parts.length === 0) parts.push("nothing was open to close");

        return parts.join(", ") + ".";
      },

      // === Convenience alias: just close the modal/overlay ===
      // Some agents call this instead of closeAll. Makes the surface more
      // forgiving when the LLM picks a "natural" name.
      closeOverlay: () => {
        if (typeof document !== "undefined" && document.fullscreenElement) {
          document.exitFullscreen().catch(() => { /* noop */ });
        }
        ...
        return "No overlay was open.";
      },

      // === Convenience alias: stop the video ===
      // Specific for video — clearer intent when user says "stop the video"
      stopVideo: () => {
        if (typeof document !== "undefined" && document.fullscreenElement) {
          document.exitFullscreen().catch(() => { /* noop */ });
        }
        if (externalContent?.kind === "video") {
          setExternalContent(null);
          return "Video stopped and closed.";
        }
        return "No video was playing.";
      },
    },
    onConnect: () => {
      setConnecting(false);
